...
| 2026-05-07 | 批次模式補上後綴替換：新增 `_infer_suffix()` 偵測欄位的單字母國家代碼後綴（如 `(w)`/`(t)`），批次 Word 迴圈加入 `_b_raw_suffix`/`_b_tgt_suffix` 替換邏輯，與單一模式對齊 |
| 2026-05-07 | 修正歷史紀錄重複記錄問題：Word 多表格輸出改為整次轉檔只記一筆，`raw_file` 欄位列出所有用到的不重複資料來源（以 `, ` 串接），`rows`/`mapped`/`total_cols` 改為各表格合計值 |
| 2026-05-07 | 統一資料來源 label 格式：單一 Excel 選取多工作表時，label 由僅工作表名改為 `檔名／工作表名`，與多檔上傳格式一致，歷史紀錄顯示完整來源資訊 |
